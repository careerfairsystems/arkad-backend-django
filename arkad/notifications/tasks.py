...
from student_sessions.models import StudentSession, SessionType, StudentSessionApplication
...
def notify_event_tomorrow(user_id: int, event_id: int) -> None:
    ...
    if event.verify_user_has_ticket(user_id):
        fcm.send_to_token(token, f"Påminnelse: {event.name} är imorgon!",
...
def notify_event_one_hour(user_id: int, event_id: int) -> None:
    ...
    event: Event = Event.objects.get(id=event_id)
    if event.verify_user_has_ticket(user_id):
        fcm.send_to_token(token, f"Påminnelse: {event.name} är om en timme!",
...
def notify_student_session_tomorrow(user_id: int, student_session_id: int) -> None:
    ...
    try:
        application = StudentSessionApplication.objects.get(user_id=user_id, student_session_id=student_session_id)
        if application.is_accepted():
            if session.session_type == SessionType.REGULAR:
                fcm.send_to_token(token, f"Påminnelse: Student session med {session.company.name} är imorgon!",
                                  f"Glöm inte din student session med {session.company.name} imorgon!")
            elif session.session_type == SessionType.COMPANY_EVENT:
                fcm.send_to_token(token, f"Påminnelse: Företagsevent med {session.company.name} är imorgon!",
...
def notify_student_session_one_hour(user_id: int, student_session_id: int) -> None:
    ...
    try:
        ...
        if application.is_accepted():
            if session.session_type == SessionType.REGULAR:
                fcm.send_to_token(token, f"Påminnelse: Student session med {session.company.name} är om en timme!",
                                  f"Glöm inte din student session med {session.company.name} om en timme!")
            elif session.session_type == SessionType.COMPANY_EVENT:
                fcm.send_to_token(token, f"Påminnelse: Företagsevent med {session.company.name} är om en timme!",
...
def notify_event_registration_open(event_id: int) -> None:
    ...
    # Send by topic
    event: Event = Event.objects.get(id=event_id)
    fcm.send_to_topic("broadcast", f"Anmälan för {event.name} har öppnat!",
...
def notify_student_session_registration_open(student_session_id: int) -> None:
    session: StudentSession = StudentSession.objects.get(id=student_session_id)
    if session.session_type == SessionType.REGULAR:
        fcm.send_to_topic("broadcast", f"Anmälan för student session med {session.company.name} har öppnat!",
                          f"Reservera en plats till student session med {session.company.name} nu! Öppna Arkadappen för att anmäla dig.")
    elif session.session_type == SessionType.COMPANY_EVENT:
        fcm.send_to_topic("broadcast", f"Anmälan för företagsevent med {session.company.name} har öppnat!",
...
def notify_event_registration_closes_tomorrow(event_id: int) -> None:
    ...
    fcm.send_to_topic("broadcast", f"Anmälan för {event.name} stänger imorgon!",
...
def notify_student_session_application_accepted(user_id: int, student_session_id: int) -> None:
    ...
    if session.session_type == SessionType.REGULAR:
        fcm.send_to_token(user.fcm_token, f"Du har blivit antagen till en student session med {session.company.name}!",
                          f"Grattis! Du har blivit antagen till en student session med {session.company.name}, kolla i appen för mer info.")
    elif session.session_type == SessionType.COMPANY_EVENT:
        fcm.send_to_token(user.fcm_token, f"Du har blivit antagen till ett företagsevent med {session.company.name}!",
