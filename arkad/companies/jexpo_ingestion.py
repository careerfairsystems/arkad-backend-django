...
class StudentSession(BaseModel):
    ...
    requests: Optional[bool] = Field(None, alias="$requests")  # type: ignore
...
class ExhibitorSchema(BaseModel):
    # Some are left out
    index: List[str] = Field(default_factory=list, alias="$index")  # type: ignore
    ...
    emails: List[Email] = Field(default_factory=list, alias="$emails")  # type: ignore
    tokens: List[Token] = Field(default_factory=list, alias="$tokens")  # type: ignore
    ...
    rev: Optional[datetime] = Field(None, alias="$rev")  # type: ignore
    key: Optional[str] = Field(None, alias="$key")  # type: ignore
