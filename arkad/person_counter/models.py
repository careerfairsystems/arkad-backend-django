...
class PersonCounter(models.Model):
    ...
    room = models.ForeignKey(
        RoomModel, on_delete=models.CASCADE, related_name="counters"
    )
    ...
    def get_last(cls, room_name: str) -> Optional["PersonCounter"]:
        return (
            cls.objects.filter(room__name=room_name)
    ...
    @classmethod
    def add_delta(
        cls, room: RoomModel, delta: int, updated_by: Optional[User] = None
    ) -> "PersonCounter":
        """
        ...
        with transaction.atomic():
            ...
            locked_room: RoomModel = RoomModel.objects.select_for_update().get(
                pk=room.pk
            )
            last: Optional[PersonCounter] = (
                cls.objects.filter(room=locked_room)
                .order_by("-created_at", "-id")
                .first()
            )
            new_count: int = (last.count if last else 0) + delta
            return cls.objects.create(
                room=locked_room, count=new_count, delta=delta, updated_by=updated_by
            )

    @classmethod
    def reset_to_zero(
        cls, room: RoomModel, updated_by: Optional[User] = None
    ) -> "PersonCounter":
        """
        ...
        with transaction.atomic():
            ...
            if current_count == 0:
                ...
                return cls.objects.create(
                    room=locked_room, count=0, delta=0, updated_by=updated_by
                )
            return cls.objects.create(
                room=locked_room, count=0, delta=-current_count, updated_by=updated_by
            )
